...
QEMU-smoke + bats покрывают POSIX/busybox-совместимость и базовую функциональность. Но **физический Wi-Fi, USB-tether, реальный AmneziaWG-handshake и поведение при reboot** — проверить можно только на железе. Этот документ — single source of truth для такого прохода.
...
## Phase 7 — Beryl AX hardware-специфика (~10 мин)

### 7.1. USB-порт для tether (использование после установки)
