...
#
# Кавычки снимаем ТРЕМЯ отдельными подстановками, а не одной с двумя `['\"]*`: в links.txt значения
# уже закавычены, а жадный `\(.*\)` захватывал закрывающую кавычку внутрь значения — поверх неё
# дописывалась своя, и в строке оказывалось ТРИ кавычки. Такой файл при `. links.env` падает не
# сразу и не понятно («command not found» на середине ссылки), а третьим шагом прогона.
printf '%s\n' "$raw" \
    | grep -E '^(VLESS_REALITY|HYSTERIA2|HYSTERIA2_PORT_HOPPING|AWG_CONF_REMOTE)=' \
    | sed -e "s/=['\"]/=/" -e "s/['\"]$//" -e "s/=\(.*\)$/='\1'/" > "$OUT"
chmod 600 "$OUT"

n="$(wc -l < "$OUT")"
[ "$n" -ge 2 ] || { echo "✗ в links.txt нашлось всего $n ссылок — стенд поднялся не полностью"; exit 1; }
# Проверяем файл на разбор ЗДЕСЬ: он пишется один раз, а читается всеми шагами прогона, и битое
# квотирование иначе всплывает далеко от причины.
sh -n "$OUT" 2>/dev/null || { echo "✗ $OUT не разбирается shell'ом — сломано квотирование значений"; exit 1; }
