#!/bin/bash
# tests/qemu/webui.sh — T3b: HTTP-слой веб-мастера через настоящий uhttpd.
#
# Расширение smoke (T3a) тем, что юниты движка и ubus-смоук проверить НЕ могут:
# путь БРАУЗЕРА. Тот же транспорт, что у web/src/lib/ubus.js — HTTP POST /ubus
...
# полный install (T3c: install.sh).
#
# NOT hermetic — нужен интернет в VM для apk add uhttpd-mod-ubus.
# Запуск: make qemu-webui. Время: ~3 мин с KVM.
...
# ─── HTTP / JSON-RPC helpers (как в web/src/lib/ubus.js) ──────────────────
http_ubus() {
...
echo "→ Раскладываю движок и web-бандл (как пакет)"
...
# к VM (урок smoke: rootpass строго последним). Целостность проверяем по HTTP:
...
echo
echo "✅ T3b ЗЕЛЁНЫЙ: uhttpd раздаёт бандл, ACL держит границу anon/admin,"
