#!/bin/bash
# tests/qemu/hysteria-v2.sh — T3e-v2: Hysteria2 (Full-тир) data-plane WIRING на живом OpenWrt.
#
# Зачем (чего НЕ покрывают юниты, reality-v2 и install-v2):
...
# Замер пропускной способности под потерями — отдельный стенд (make qemu-netem-v2).
#
# Запуск: make qemu-hysteria-v2 (нужен интернет для apk). ~4-6 мин с KVM.
...
apk_try() { # до 10 попыток по 10с, тихо (флап зеркала не красит тест)
    local cmd="$1"
...
echo ""
echo "✓ T3e-v2 HYSTERIA2 WIRING ЗЕЛЁНЫЙ: сборка умеет hysteria2, конфиг+port hopping+маршрут+TUN"
echo "  применяются, битый конфиг отвергается до подмены рабочего, проба отвергает мёртвый туннель,"
echo "  teardown чистит. Пропускная способность под потерями — make qemu-netem-v2."
