...
Расширяет T3b: вместо «положить только handler» — заливает весь репо в `/opt/cheburnet/` (как это делает `bootstrap.sh` на роутере) и **запускает `setup/install.sh` целиком**:
