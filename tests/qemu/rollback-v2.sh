#!/bin/bash
# tests/qemu/rollback-v2.sh — T3g-v2: ПОЛНАЯ установка через ubus на живом OpenWrt и ОТКАТ.
#
...
#     часть покрыта юнитами, импурная — создание секций network, ifup, route_allowed_ips — нет.
...
# Запуск: make qemu-rollback-v2 (нужен интернет для apk). ~5-8 мин с KVM.
...
# install-v2.sh). if, не `[ … ] && …`: ловушка set -e (CLAUDE.md).
apk_try() {
    local cmd="$1"
...
echo
echo "✓ T3g-v2 pass — полная установка и откат на живом OpenWrt:"
