#!/bin/bash
# tests/qemu/reboot-v2.sh — T3h-v2: data-plane ПЕРЕЖИВАЕТ ПЕРЕЗАГРУЗКУ роутера.
#
...
# систему: fw4 reload покрыт (install-v2), а полная перезагрузка со всеми init-скриптами, hotplug'ами
...
#
# Запуск: make qemu-reboot-v2 (нужен интернет для apk). ~6-9 мин с KVM.
...
if [ "$DOH_BEFORE" = "yes" ]; then
    vm_ssh "nslookup example.com 127.0.0.1 >/dev/null 2>&1" \
        || { echo "  ✗ резолв РАБОТАЛ до ребута и перестал после — регресс персистентности DNS";
