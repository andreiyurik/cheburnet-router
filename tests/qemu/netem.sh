#!/bin/bash
# tests/qemu/netem.sh — ЗАМЕР: держит ли Hysteria2 (QUIC) скорость на канале с потерями.
#
...
#
# Запуск: make qemu-netem (нужен интернет для apk). ~6-10 мин с KVM.
...
echo "→ Раскладываю движок и конфиги стенда"
...
echo ""
echo "✓ T3f NETEM-СТЕНД ОТРАБОТАЛ: цифры выше — материал для ADR 0004, раздел «Замеры»."
