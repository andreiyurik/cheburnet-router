#!/bin/bash
# tests/qemu/smoke.sh — T3a hermetic VM smoke для движка (ucode).
#
...
#     паролем (самое слабое допущение: дефолтный rpcd даёт root-сессии права).
#   • steps/wifi: честный no-op на VM без радио.
#   • (doh/выбор DNS-провайдера — в T3c: нужен пакет https-dns-proxy с его секцией 'config'.)
...
#
# Запуск: make qemu-smoke  (или ./tests/qemu/smoke.sh из корня репо).
...
# ─── деплой движка (как пакет: shim + engine без tests/README + ACL) ──────
...
# создаётся пакетом — поэтому он проверяется в T3c (install.sh, пакет установлен), а не
...
# Самое слабое допущение (вход в панель): дефолтный rpcd пускает root по
...
echo
echo "✓ T3a smoke pass — движок работает на реальном OpenWrt $OPENWRT_VERSION:"
