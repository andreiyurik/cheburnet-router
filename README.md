...
### Раздельное туннелирование из коробки: сайты `.ru` — напрямую, остальные — через VPN. Автоматически.

Роутер сам решает по каждому запросу: **домашняя зона (по умолчанию — вся `.ru` + ваш
список) идёт напрямую** — полная скорость, реальный IP, банки и локальные сервисы работают
как обычно. **Весь остальной трафик — в зашифрованный туннель AmneziaWG.** Один роутер
...
- ⚡ **Раздельное туннелирование — главная фишка.** Зона `.ru` и добавленные вами сайты идут
  **напрямую**: полная скорость, реальный IP — банки, госсервисы и доставки работают без капч
  и странностей. Всё остальное — через VPN. Автоматически, без ручных списков на каждый сайт.
...
|---|---|
| Под VPN локальные сайты тормозят, банки ловят капчи и блокируют вход. | Зона `.ru` и твой список идут напрямую — полная скорость, реальный IP. Автоматически. |
...
flowchart LR
    ...
    Choice -->|"Зона .ru + ваш список"| Direct["⚡ Напрямую<br/>(полная скорость, реальный IP)"]
...
Роутер сам решает по каждому запросу, куда его отправить. Одна запись зоны покрывает сразу все
сайты в ней: дефолтная `ru` — это и банк, и госуслуги, и доставка, ничего не нужно перечислять
руками; отдельные сайты добавляются одной строкой в панели. **Весь остальной** трафик — в
...
На сайты зоны `.ru` и вашего списка — нет, они идут напрямую через провайдера. На остальные — зависит от качества VPN-сервера и твоего канала; AmneziaWG работает на уровне ядра ОС, потолок для бытового роутера обычно 200–500 Мбит/с. Для видео, мессенджеров и веба разница не ощущается.
...
Включается kill-switch — трафик не утечёт в открытую сеть. Сайты зоны `.ru` и вашего списка продолжают работать напрямую. В веб-панели видно, что VPN упал, и можно одним кликом перезапустить или сменить `.conf`.
