...
> **Красное предупреждение `WARNING: REMOTE HOST IDENTIFICATION HAS CHANGED`?** Не взлом — просто
> `192.168.1.1` раньше принадлежал другому устройству, и компьютер запомнил старый ключ.
> Выполните `ssh-keygen -R 192.168.1.1` и подключитесь заново.
...
одним полем в панели, без перезагрузки. Домен (`example.com`) покрывает и все его поддомены.
...
после основной работы. Проект бесплатный и таким останется — но если он сэкономил вам время, нервы
или деньги, самое время вернуть немного обратно.
