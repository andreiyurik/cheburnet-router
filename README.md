...
### Образовательный проект: собери OpenWrt-роутер со split-tunnel за вечер — и пойми, как он устроен.

Демонстрация возможностей современного open-source-стека для роутеров.
Превращает [совместимый OpenWrt-роутер](#совместимое-железо) в учебный стенд,
который решает и практическую задачу: настраиваемый split-tunnel для всей семьи.
Домены из вашего direct-списка идут напрямую, весь остальной трафик — через
зашифрованный VPN-туннель. Реклама и трекеры не загружаются.

Каждая технология стандартная и переносимая — изучив её здесь, вы сможете
применить знание на любом Linux/OpenWrt. Весь код открыт; опытный пользователь
донастроит роутер под свои задачи через LuCI или SSH.

> 🚧 Идёт разработка **v2** — более лёгкой и надёжной версии для слабых роутеров (ветка `feat/v2`).
...
Две цели, и обе главные.

**Учебная.** Split-routing, VPN, DNS-over-HTTPS, kill-switch, firewall — про эти
технологии обычно читают в теории. Здесь вы собираете их в работающую систему
своими руками и видите каждый шаг: `docs/` объясняют не только «какую команду
ввести», но и «почему это работает».

**Практическая.** Настроить **один** роутер так, чтобы все домашние устройства
...
- **Домены из direct-списка** — напрямую через провайдера. Без задержки VPN,
  на полной скорости. Что в списке — решаете вы: обычно локальные сайты и
  сервисы с гео-привязкой (банки, стриминг), которым VPN мешает.
...
> **🌍 Зеркало пакетов недоступно?** В некоторых сетях `downloads.openwrt.org` недоступен или сильно ограничен. Установщик это сам обнаружит за 5 секунд и подскажет — но если хочешь подготовиться заранее, прочти **[docs/install-blocked.md](docs/install-blocked.md)** (2 способа дать роутеру временный доступ: ноутбук с VPN или Android+USB+AmneziaVPN).
...
> Из вашей сети недоступно зеркало пакетов OpenWrt. Пока cheburnet не установлен, помочь себе он не может — нужно на 10 минут установки дать роутеру интернет через сторонний VPN. Подробно (2 схемы, шаги):
>
...
**VPN-сервер:** свой VPS или подписка совместимого провайдера.

**Проект не привязан к провайдеру** — подойдёт любой стандартный AmneziaWG-конфиг
(`.conf`): собственный сервер на VPS (стек Amnezia открыт, документация публичная)
или готовая подписка. Пример совместимого провайдера:

<div align="center">

### 👉 [Amnezia Premium — пример совместимого провайдера, скидка 15% →](https://storage.googleapis.com/amnezia/amnezia.org?m-path=premium&arf=EB5KDKXCJYQYP4MG&coupon=CHEBURNET15)

*От 325 ₽/мес на 7 устройств. Промокод `CHEBURNET15` — скидка 15%, поддерживает развитие проекта.*

</div>

**Почему AmneziaWG.** Это форк WireGuard с обфускацией: маскирует сетевую сигнатуру
протокола, сохраняя его скорость и простоту. Протокол и клиенты открытые — отличный
учебный материал о том, как устроены современные VPN.

**Без своего VPS или подписки не получится** — VPN-туннелю нужен сервер, и кто-то
платит за его трафик. Бесплатных вариантов с приличной скоростью не бывает.
...
На домены из direct-списка — нет, они идут напрямую через провайдера. На остальные — зависит от качества VPN-сервера и твоего канала; AmneziaWG работает на уровне ядра ОС, потолок для бытового роутера обычно 200–500 Мбит/с. Для просмотра видео, мессенджеров и веба разница не ощущается.
...
Включаются три независимых слоя kill switch — трафик не утечёт в открытую сеть. Домены из direct-списка продолжают работать. В веб-панели видно, что VPN упал, и можно одним кликом перезапустить или сменить `.conf`. Подробнее: [docs/08-killswitch.md](docs/08-killswitch.md).
...
Не «лучше», а **по-другому**. Коммерческий VPN — закрытое приложение на каждом устройстве, всё доверяешь одной компании. Здесь — открытый код на твоём роутере: одно подключение к Wi-Fi покрывает все устройства в доме, включая Smart TV. Плюс настраиваемый split-routing — ваш direct-список напрямую, остальное через VPN — коммерческие сервисы редко это делают.

Минус: нужен свой VPN-сервер. Либо подписка совместимого провайдера, либо поднять самостоятельно на VPS.
...
Если нужен полный туннель (весь трафик через VPN, без direct-исключений) — есть **TRAVEL-режим**: переключается одной командой `vpn-mode travel` или из веб-панели. Подробнее: [docs/07-modes.md](docs/07-modes.md).
...
Это значит, что из твоей сети недоступно зеркало пакетов OpenWrt (`downloads.openwrt.org`). Пока cheburnet не установлен, помочь себе он не может: chicken-and-egg.

Решение — на время установки дать роутеру интернет через VPN. Два готовых варианта (выбор по железу):
...
Бывает нужно для любого сервиса с гео-привязкой: банк, государственный портал, локальный стриминг. Добавляем домен в исключения podkop через LuCI:
...
### 🔗 2. Нужен VPN-сервер? Возьми Amnezia Premium со скидкой 15% — [промокод CHEBURNET15](https://storage.googleapis.com/amnezia/amnezia.org?m-path=premium&arf=EB5KDKXCJYQYP4MG&coupon=CHEBURNET15)

Пример совместимого провайдера (подойдёт любой AmneziaWG-конфиг). Промокод уже встроен в ссылку: тебе −15% к цене, проекту — поддержка.
...
- **Без VPN-сервера работает только локально:** блок рекламы и прямой роутинг direct-списка.
...
flowchart LR
    ...
    Split -->|"direct-список"| ISP["🌐 Локальный ISP<br/>напрямую"]
...
- **FakeIP.** Sing-box подменяет DNS-ответ для доменов direct-списка на адрес из служебного диапазона `198.18.0.0/15`. Клиент коннектится на этот fake-IP → ядро через TPROXY отдаёт пакет sing-box'у → тот по таблице кеша вспоминает домен и отправляет в `direct-out` (WAN). Без FakeIP домен теряется в TCP-пакете, и принять решение «куда отправить» становится невозможно.
...
| **Podkop + sing-box** | Policy-based routing на FakeIP: direct-список напрямую, остальное через VPN | [docs/03](docs/03-podkop-routing.md) |
...
vpn-mode home      # direct-список напрямую + остальное через VPN (по умолчанию)
...
Проект образовательный: демонстрация возможностей современного open-source-стека
для роутеров и рабочий стенд для изучения сетевых технологий — AmneziaWG/WireGuard,
DoH, split-routing, nftables. Эти технологии стандартные, применяются в корпоративных
сетях, банкинге и при работе в публичных Wi-Fi.

Содержимое direct-списка и назначение туннеля — настройки пользователя.
Применять эти технологии законно в большинстве стран, но ответственность за
соблюдение местного законодательства — на пользователе. Проект не предназначен
для нарушения законов вашей юрисдикции.
