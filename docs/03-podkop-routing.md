...
Состояние режима живёт в UCI подkop'а (`podkop.exclude_ru.community_lists`): заполнено = HOME, очищено = TRAVEL. Persistent через перезагрузку и sysupgrade.
...
- **Web-UI**: `http://192.168.1.1/cheburnet/` → кнопка HOME/TRAVEL (RPC `mode_switch`)
- **Вручную**: `vpn-mode home | travel | status`

После reboot подkop сам стартует с сохранённой UCI-конфигурацией, mode восстанавливается без отдельного init.d-сервиса.
