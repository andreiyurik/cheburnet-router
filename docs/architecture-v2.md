...
## 🌅 Sunset v1 — когда и как удаляем старый слой

v1 (`web/`, `setup/`, `lib/`, `tests/integration`, `tests/qemu`-части, `AGENTS.md`,
v1-главы `docs/`) **заморожен**: только фиксы стабильности, новые фичи — в v2. Удаляем не
«когда v2 написан», а «когда v2 взял нагрузку» (суть strangler-fig). Чек-лист удаления:

- [ ] v2 прошёл QEMU **и** хотя бы один реальный роутер: сквозная установка через мастер,
      трафик реально едет (split + туннель + NAT), replace_awg_conf / factory_reset /
      install_cancel отработали живьём.
- [ ] v2-ветка смержена в master, опубликован feed + Release.
- [ ] Решена судьба v1-фич, которых нет в v2: watchdog (`08`), ssh-hardening (`09`),
      SQM/conntrack (`10`) — перенесены в движок **или** явно отброшены записью здесь.
- [ ] Есть путь для существующих v1-установок: миграция или честное «чистая переустановка»
      в release notes.
- [ ] Образовательные уроки v1-глав docs/ (threat model kill-switch, DNS-цепочка, режимы,
      Wi-Fi-грабли) перенесены в `docs/v2`. Целиком выбрасывается только
      `03-podkop-routing.md` (sing-box-специфика).

После выполнения всех пунктов v1 удаляется **отдельным коммитом** (git-история всё хранит,
потеря невозможна). До тех пор v1 — рабочий fallback и reference-реализация оплаченных
инцидентами решений.
