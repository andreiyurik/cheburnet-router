...
Этот раздел — про установку с нуля и про то, когда `awg show awg0 | grep handshake` пуст или показывает старый handshake. Скрипт `setup/01-amneziawg.sh` сам пытается **три fallback'а** (без I1 → без S3/S4 → нормализация H-диапазонов), но иногда автоматики недостаточно — здесь разбор по симптомам.
...
3. **H-диапазоны (свежие self-hosted Amnezia с ~2025 года)** — если в вашем `.conf` строки H1-H4 выглядят как `H1 = 213093219-313093218` (с дефисом), это AWG 2.0-формат с рандомизацией header-байтов. Если AWG-tools на OpenWrt их пока не поддерживает или сервер ожидает single value — handshake не сходится. Скрипт делает третий fallback автоматически: нормализует диапазоны до первого значения. Руками:
   ```bash
   # Берём первое число из диапазона. Например, "213093219-313093218" → "213093219".
   uci set network.awg0.awg_h1='213093219'
   # (аналогично для h2/h3/h4)
   uci commit network && ifdown awg0 && sleep 2 && ifup awg0
   ```
   Минус — пропадает рандомизация header-байтов между пакетами, обфускация чуть слабее. Туннель работает.
...
**Если установка падает на `apk add` с «Operation not permitted»** — провайдер скорее всего фильтрует загрузку конкретных пакетов (sing-box и т.п. — частая практика DPI). Обход — установка через мобильный интернет с AmneziaVPN, одной командой. См. [docs/install-blocked.md](install-blocked.md).

**Если установка падает с «РОУТЕР НЕ ПОДХОДИТ»** на preflight — у вашего железа меньше flash/RAM чем нужно. Альтернативная схема (без подкопа) — см. [docs/router-too-small.md](router-too-small.md).
