...
- [ ] **Паритет фич v2 ≥ v1.** Всё, что обещано пользователю, работает в v2:
  ...
  - [ ] **VLESS+Reality (Full-тир) — ⛔ блокер релиза.** Пока не построен Full-тир
        (PoC sing-box в QEMU → шаг движка → ось протокола в UI → preflight-гейт по
        железу), v2 **не достигает паритета цели** и v1 удалять нельзя.
