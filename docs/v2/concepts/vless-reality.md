...
флеша** (установленный `sing-box` весит ~42 МБ — замер `make qemu-reality-v2`) **и устанавливаемый
...
> Как и в Light-тире: `MemTotal` меньше физической памяти на kernel-reserve, поэтому порог —
> **240 МБ** («плата на 256 МБ»). Прежние 256 не пропускали НИ ОДИН 256-МБ роутер: Full молча
> не предлагался ни в мастере, ни в панели. На рекомендуемых 512-МБ платах это не проявлялось.

Точные пороги подтверждаются замером на реальном железе; вес `sing-box` меряет
`make qemu-reality-v2` и сравнивает с порогом флеша (см. [[0004-multi-protocol-tiers]]).
