...
### Проблема: DPI детектирует стандартный WireGuard
...
**Важно:** новые параметры (`I1-I5`, `S3`, `S4`) в [Interface] секции **опциональны со стороны клиента** — документация Amnezia прямо разрешает их не отправлять: *"If none of I1 through I5 are set, no CPS packets are sent"*. Cross-version контракт «клиент без X ↔ сервер с X» в доке явно **не описан**; на практике серверы AWG 2.0 принимают соединение без I1 (это decoy-пакет перед handshake, не часть крипты), но гарантий нет. Старые `Jc/Jmin/Jmax/S1/S2/H1-H4` — обязательные и **должны точно совпадать** между клиентом и сервером. Источник: [Amnezia docs](https://docs.amnezia.org/documentation/amnezia-wg/).

Если AWG-сервер у вас на собственном VPS, держите версию `amneziawg-go` свежей — иначе клиентский `.conf` с полями v2.0 отдаёт handshake неверной длины, сервер тихо отбрасывает. Симптом и лечение — в [docs/09-troubleshooting.md](09-troubleshooting.md), раздел «AmneziaWG: установка падает / handshake не идёт». В `setup/01-amneziawg.sh` встроены два fallback'а (снять `I1`, снять `I1+S3+S4`), которые автоматически срабатывают, если первая попытка не довела туннель до handshake.
