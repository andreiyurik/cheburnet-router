...
Отказ «не подходит, до свидания» — плохой ответ, когда не хватает пары мегабайт. Поэтому провалы
делятся по severity (см. [[hardware-requirements]]): **hard** (arch/версия/пакеты/LAN-конфликт)
остаются честным отказом — пропуск обещал бы невозможное; **soft** (флеш/RAM) экран отмечает «!» и
разворачивает в разбор: чем это грозит и что можно сделать **вместо** риска (освободить флеш,
extroot, `zram-swap`).
