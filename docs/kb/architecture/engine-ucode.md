...
[ucode](https://ucode.mediatek.org/) — встроенный в OpenWrt язык (на нём написаны `fw4`,
`rpcd`, новый LuCI — не наш экзотический выбор, а системный язык платформы). Даёт то, чего нет
у shell: типы, исключения, JSON, нативные биндинги `uci`/`ubus`/`fs` — при этом
**интерпретируемый**, без кросс-компиляции под arch и без роста флеша. Подробное сравнение с
Go — [[0002-ucode-over-go]].
...
главная причина ухода от bash. См. пирамиду тестов в [[reliability]] и [docs/architecture.md](../../architecture.md).
