...
### 5. Управление (web-UI / CLI)

**Задачи:**
- Дать пользователю простой контроль режима без специального железа

**Компоненты:**
- Web-UI `http://192.168.1.1/cheburnet/` — кнопки HOME/TRAVEL через RPC `mode_switch`.
- CLI `vpn-mode` (home / travel / status) — для автоматизации и SSH-привычных пользователей.
- Состояние хранится в UCI подkop'а; после reboot подkop сам стартует с сохранённой конфигурацией — нашего init.d-сервиса не требуется.
...
❌ **Но DPI** распознаёт его по фингерпринту handshake-пакета (первые 4 байта стабильны: `0x01 0x00 0x00 0x00`). AmneziaWG добавляет **junk-пакеты** (Jc/Jmin/Jmax), **случайное padding** (S1/S2) и **заголовки-обманки** (H1-H4), чтобы handshake выглядел как случайный UDP-шум. Подробно: [docs/02](02-amneziawg.md).
