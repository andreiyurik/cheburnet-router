...
1. **Недоступность upstream-хоста (v1).** `raw.githubusercontent.com` в некоторых сетях
   периодически недоступен. У пользователя без рабочего VPN (а на этапе установки VPN ещё
   нет!) `wget` молча зависает на 60 сек, потом fail. Поэтому `setup/02-podkop.sh` и
   `setup/03-adblock.sh` сначала пробуют upstream, при ошибке — берут локальную копию отсюда.
