...
`setup.sh` проводит пользователя через 4 шага (адрес роутера → .conf → Wi-Fi → подтверждение), `rsync`'ит репо в `/opt/cheburnet/` на роутере и запускает там единственный оркестратор `setup/install.sh`. Веб-мастер (через корневой `install.sh` + RPC `install_start`) запускает тот же самый `setup/install.sh`.
...
Перед запуском шагов `setup/install.sh` раскладывает файлы по финальным путям через **`setup/manifest.txt`** — это единственный источник правды о том, что копируется на роутер. Добавил новый runtime-скрипт или конфиг? — добавь строку в манифест, не плодите `cp` в setup-шагах.
