...
const REGISTRY = [
	...
	// Install-токен для повторной настройки — ТОЛЬКО вошедшему владельцу, и выпускается, если его
	// нет. Успешная установка токен снимает (одноразовый), поэтому и «Настроить заново», и мастер
	// после сброса упирались в «токен не найден — запустите bootstrap по SSH»: стена для того, кто
	// в SSH не ходит. Прав это никому не добавляет — у admin-сессии есть пароль root, которым тот
	// же файл пишется по SSH; зато выпуск происходит по осознанному действию владельца, а не
	// лежит пропуском «на всякий случай».
	// access=write, а не read: метод СОЗДАЁТ состояние, и классифицировать его как чтение было бы
	// нечестно по отношению к ACL. auth=admin обязателен: попади он в anon-тир, любой в LAN
	// выписал бы себе право пройти мастер на чужом роутере.
