...
Дефолт — **`hagezi:pro`** (реклама + трекеры, сбалансированно). Не «аггро»-фильтрация:
цель — «у бабушки ничего не отвалилось», а не максимум блокировок.

## Тир: кто чем владеет в `raw_block_lists`

Тир (`opts.tier`, ubus `set_blocklist_tier`) владеет **только hagezi-шорткатом**; остальные
токены — raw-URL'ы (NSFW-лист [семейного режима](../family/)) — **сохраняются** при смене тира.
Это фикс бага v1, где `sed` по всей строке тихо выключал NSFW-блок. Семантика тира:

- явный `opts.tier` → применяется;
- не задан → **сохраняется текущий** из конфига (переустановка не сбрасывает выбор админа);
- чистая система → дефолт `pro`.

`parse_blocklists(config)` → `{tier, extras}` — этим же разбором `status` отдаёт текущий тир в UI.
