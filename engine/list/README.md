...
community, а не ведём руками (architecture). Движок периодически тянет его и регенерит
...
ucode -R engine/list/fetch.uc                                       # на роутере, дефолтный источник
ucode -R engine/list/fetch.uc https://example.org/direct-list.txt   # свой источник
```
