// list.uc — импорт community-списка доменов ПРЯМОГО доступа (direct). Чистое ядро: сборка списка
// доменов из сырого текста + пользовательского списка (валидация — из routing.uc). Загрузка по
// сети — fetch.uc (импурно, QEMU). Результат → build_plan.
//
// ИНВАРИАНТ: импортируемый список = домены, которые идут НАПРЯМУЮ (как и пользовательский
// direct-список) — см. [[split-routing]]. Перепутать «direct» и «через туннель» — источник багов;
// fail-safe страхует: промах → трафик уйдёт в туннель, не утечёт.
