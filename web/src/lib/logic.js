...
// Куда писать, если не работает. Одно место на весь UI — контакт мелькает на трёх экранах, и
// разъехавшиеся копии означают, что часть роутеров указывает в пустоту.
//
// WHY страница проекта рядом с личным контактом: ссылка, вшитая в прошивку, живёт годами. Канал
// связи может измениться, а роутеры не перепрошиваются — поэтому страница указана как
// долговечный ориентир (её содержимое правится без выпуска новой версии), а Telegram — быстрый
// путь сегодня.
...
// why — ОДНА строка, видна сразу у всех трёх вариантов одновременно (выбор идёт по симптому).
// whyMore — остальное, под «Подробнее»: три развёрнутых описания рядом человек не читает, а
// пролистывает, и выбор от этого не становится точнее.
// sniff/sniffError — дешёвая проверка «это вообще тот формат?» ДО отправки. Движок всё равно
// валидирует, но его отказ стоит трёх минут установки и отката: ссылку вместо .conf лучше
// поймать в форме.
...
// explainFail(reason, protocol?) → { error, advice } — адресная диагностика по машинному коду
// исхода (install_progress.reason). error=null у генерик-ветки: компонент сохраняет свой текст
// («Установка не удалась» / «Установщик аварийно завершился»). protocol — только чтобы не
// показывать AWG-пользователю совет про xray-core/3x-ui, который к нему не относится.
// Главный кейс: health:* — роутер настроен ПРАВИЛЬНО, но что-то не поднялось ДО коммита. Раньше
// все health-провалы схлопывались в один текст «сервер молчит, проверьте подписку» — а причиной
// мог быть баг на самом роутере (маршрут/процесс), и человек чинил не то. Здесь код и совет
// обязаны совпадать 1:1 (см. install.uc.decide_outcome): «health:dns» и «health:tunnel:process/
// route» — это НЕ про сервер, туда где написано «сообщите нам», а не «проверьте подписку».
