...
test('мастер: полный проход от проверки до панели управления', async ({ page }) => {
  ...
  // Шаг 2: настройка. Дефолты: direct-список 'ru', токен из ссылки.
  await expect(page.getByText('Шаг 2 из 4')).toBeVisible();
  await expect(page.getByLabel('Домены прямого доступа')).toHaveValue('ru');
  ...
  await expect(page.getByText('HOME (split)')).toBeVisible();
});
